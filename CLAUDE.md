...
**SpendGaugeAI hard budget enforcement (opt-in, added 2026-07-27)** — set `SPENDGAUGEAI_ENFORCE=1` to turn reporting-only into actual enforcement: `_spendgauge_budget_exceeded()` calls `spendgauge_client.acheck_budget()` (SpendGaugeAI's `GET /usage/budget`, its hard-spend-cap feature — see that repo's `docs/DESIGN.md` §8b) at the very top of `/chat` and `/stream`, right after `_validate_attachment()` and before any session/history mutation or the `tool_runner` call. If the SpendGaugeAI-configured budget is exhausted, the route raises `HTTPException(402, ...)` immediately — a normal JSON error for `/chat`, and (matching the existing `_validate_attachment` placement comment) a normal JSON response rather than an SSE `error` event for `/stream`, since the check runs before `StreamingResponse` is built. Deliberately a **separate flag from `SPENDGAUGEAI_ENABLED`** — reporting is silent and can never surprise anyone, but blocking a real chat response is user-visible, so it needs its own explicit opt-in rather than activating automatically the moment a budget happens to be configured. `_spendgauge_budget_exceeded()` only ever returns `True` on a definitive "yes, blocked" from SpendGaugeAI; disabled, not opted in, a timed-out check, or SpendGaugeAI being unreachable all fail open and let the real Claude call proceed — same fail-open guarantee `acheck_budget()` itself already makes. **Not enabled in production as of this writing** — `.env` has `SPENDGAUGEAI_URL`/`SPENDGAUGEAI_API_KEY` set (reporting is live) but no `SPENDGAUGEAI_ENFORCE`, so this stays fully inert until deliberately turned on.
