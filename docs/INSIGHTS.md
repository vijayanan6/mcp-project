...
## 37. A Shared Cost Column Gets Multiplied When You Explode It Against a One-to-Many Relationship

The "Cost by Tool" dashboard showed `code_execution` at 3 calls costing $0.1510 — a real, specific, oddly-precise-looking number that turned out to describe something that never happened. The real underlying data was one single turn that called `code_execution` three times within it (recorded as `tools_used = ["code_execution", "web_search", "code_execution", "web_search", "code_execution"]`, one JSON array on one row). The SQL aggregating cost by tool used `json_each()` to explode that array into one row per tool mention, then summed `estimated_cost_usd` per tool name — but `estimated_cost_usd` is a property of the *turn*, not of any individual tool call within it, so exploding the array and summing duplicated that single $0.0503 turn cost once for every repeated mention of the same tool name. Three mentions of `code_execution` in one turn's array became 3× that turn's cost, reported as if three separate $0.05 charges had occurred.

The bug was invisible by construction: the "calls" count (3) was *genuinely correct* — three real invocations did happen in that turn — so the number sitting right next to the wrong one looked completely trustworthy, and nothing about the inflated cost figure looked obviously broken; $0.1510 for 3 calls even divides out to a plausible-looking $0.0503/call average. It was only caught because a human looked at the actual number on the actual dashboard and asked "what is this," not because any test or code review flagged it — the aggregate SQL was syntactically fine and ran without error; it just answered a subtly different question than the one the dashboard was presenting it as answering.

Fixed by grouping to (turn, tool) first — attributing a turn's cost once per *distinct* tool it used — before aggregating across turns, rather than summing over every raw exploded array element. This also improved `avg_cost_usd`, which had the same problem in the opposite direction: it was previously averaging over inflated per-mention rows instead of real per-turn attributions.

The generalizable version: whenever a one-to-many relationship gets flattened for aggregation (a JSON array exploded via `json_each`, a `GROUP_CONCAT`'d list split back apart, a join that fans out rows) and one side of that relationship carries a cost/amount/total that belongs to the *parent* record rather than each individual child, summing after the explode will multiply that shared value by however many children came from the same parent. Deduplicate to one row per (parent, category) *before* the sum, not after — and don't trust that a correct-looking adjacent number (the call count here) means the number next to it is trustworthy too; verify each aggregate independently.
