...
- **Extracted a feature into a real, standalone, publishable product, not a code copy-paste** — [SpendGaugeAI](https://github.com/vijayanan6/SpendGaugeAI), its own GitHub repo with official Python and TypeScript client SDKs. Verified the actual install experience as a stranger would rather than trusting the packaging config, catching two real bugs that way; root-caused a machine-level TLS interception issue across three unrelated tools (a Python HTTP client, `curl`, `pip`) to one consistent fix instead of three separate patches; and caught a real security regression in my own first-pass fix — an insecure default that would have shipped to every future user — and reversed it before committing. Later shipped real spend-cap enforcement (block a call before it bills, not just alert after) and a second, softer policy tier (auto-downgrade to a cheaper model near the cap) — then made an explicit product-scope call after a real competitive comparison against Langfuse: stay a narrow enforcement layer rather than chase a much larger observability platform's feature set. See section 21.
- **Wiring a second real product to that same SDK surfaced four production bugs no amount of testing with hand-built fakes had ever caught** — three inside the SDK's `wrap()` itself (an unpatched SDK resource, an unpatched method a common calling pattern actually uses, and a silently mis-detected async client that reported real API calls at zero cost), plus a separate live dashboard bug caught by reading the actual numbers, not more code review. All four root-caused, fixed, and re-verified against a live server with a real API call — not assumed fixed from the diff. See sections 21–22.
- **Ran a full security audit against a product handling real financial/immigration data, then shipped and live-verified the highest-severity finding** — fail-closed shared API key auth via one middleware, catching a self-introduced dialog-stacking bug (and a self-introduced credential exposure) before calling it done, not after. Also designed the safety layer for an agent capable of sending real email; live testing surfaced a genuinely interesting failure with zero adversarial input — the model reinterpreted a rejection message's own wording as license to retry — caught by the mechanical backstop underneath regardless, then fixed at the root. See sections 23–25.
...
- **Pragya AI Assistant** — this project's AI-agent core (chat, MCP tools, RAG, multimodal attachments), extracted into a separate, independently-versioned real assistant used day to day, with its own architecture choices — including a deliberately different SpendGaugeAI integration pattern that ended up surfacing four production bugs in SpendGaugeAI itself, live-account Microsoft Graph integrations (OneDrive, Outlook) with a real agent-safety design for irreversible email sending, and a full security audit with remediation underway. See sections 22–25.
...
**Shipped real enforcement, not just alerting, once the product's own claims needed to become true.** The dashboard could already flag high spend after the fact; a hard cap that blocks a call *before* it bills was still missing. Built `GET /usage/budget` plus an opt-in `wrap(..., enforce=True)` across both SDKs, deliberately scoped global-cap-only and fail-open — a monitoring outage must never take down the real app using it — both explicit, discussed tradeoffs, not defaults picked by omission. Verified against a live server, including confirming the deployed instance itself wasn't stale, before switching enforcement on for a real consuming app.

**A real second integration — not another hand-built test double — found four production bugs no test suite had ever caught.** Wiring the same SDK's `wrap()` into a second, independent product (Pragya AI Assistant, section 22) for the first time surfaced three stacked bugs inside `wrap()` itself: it silently never patched the SDK's separate `.beta.messages` resource (where any tool-using agent's calls actually go), never patched the third method (`.parse()`) that a common agentic calling pattern uses internally instead of the obvious one, and misdetected async clients because the real SDK's internal dispatcher decorator defeats a direct `inspect.iscoroutinefunction()` check — silently capturing an *unawaited coroutine* instead of the real response and reporting all-zero cost while the actual API call kept working perfectly. Every prior test, including this project's own reporting integration, had used hand-built fake clients that happened to not exercise any of the three shapes. Found by tracing the real SDK's own source rather than guessing, reproduced each bug's exact shape in a new regression test, fixed all three, and re-verified against a live redeployed server with a real API call landing real non-zero token counts. A fourth, separate bug — a dashboard field silently scoped by a project filter it structurally shouldn't have been — was caught by directly reading the live numbers, not by more code review; owned the miss honestly rather than deflecting it.

**Shipped a second policy tier as product owner, not just an engineer taking a ticket.** Recommended and built budget-aware model downgrade: once spend nears the cap but isn't over it, `wrap()` now automatically routes calls to a cheaper model instead of only offering a binary block, deliberately reusing an existing alert threshold rather than adding new configuration surface. Verified against two separate live server instances — confirmed the new status field flips correctly server-side, confirmed both SDKs actually swap the outgoing model over a real HTTP round-trip, and confirmed the hard block still wins over the softer downgrade once spend is genuinely exhausted. Then ran a real competitive comparison against Langfuse — a funded, adjacent-space competitor — and made an explicit scope call rather than chasing feature parity: stay a narrow spend-enforcement layer, since blocking a call before it bills is the one structural thing this product does that a pure observability platform can't.

---

### 22. A Second Product, A Deliberately Different Integration Choice — Pragya AI Assistant

Extracted the AI-agent core (chat, MCP tools, RAG, multimodal attachments) out of this project into its own standalone product, Pragya AI Assistant — a fresh git repository with no shared history, not a fork or a copy-paste. This project stays the from-scratch learning sandbox; Pragya is the real assistant used day to day, which changes what "correct" means for it: it has to actually work, not just demonstrate a concept.

**Chose a structurally different SpendGaugeAI integration pattern on purpose, not by default.** This project reports usage manually, once per request, via explicit `.alog()` calls with hand-accumulated totals. Pragya instead wraps the Anthropic client directly (`wrap()`), so every underlying call an agentic tool loop makes reports — and can be budget-checked — independently, rather than once per top-level request. A deliberate tradeoff for a project built around multi-tool agentic turns rather than a single request/response cycle. This choice is exactly what exercised `wrap()` against a real SDK client for the first time and surfaced the three bugs described in section 21 — a lower-risk integration pattern would have kept masking them.

**Found two more real bugs by testing the actual running deployment, not a freshly started copy of the same code.** Every earlier verification pass ran the app directly on the host via `uvicorn`. The persistent Docker container real traffic actually goes through had silently never worked: a dependency listed only as a comment in `requirements.txt`, never really installed in the image, and a `localhost` URL that resolves to something different inside a container than on the host machine it needs to reach. Neither was visible from reading the source or from testing the direct-run path — only from testing the exact container in front of real users. Rebuilt, then verified with a real chat message moving the real tracked balance in the connected service, not just a green healthcheck.

**Why this matters:** the same lesson recurs from a new angle across sections 21 and 22 — "verified" is only as strong as what was actually exercised. A hand-built test double that looks like the real SDK client, or a freshly-started process that looks like the real deployment, can both pass every check while masking a completely different failure underneath. Building a second, independent product wasn't just feature work — it was the thing that finally forced both gaps into the open.
...
### 24. Live Cloud Integration Against a Real Personal Microsoft Account — OneDrive, On-Demand by Design

Extended Pragya with real access to a personal OneDrive over Microsoft Graph API — not a demo account, the actual live account, authorized through a real Azure app registration that itself needed live troubleshooting (a personal Microsoft account signing into the Azure Portal landed in a tenant-less default directory that can't host an app registration at all; fixed via the real tenant-provisioning signup flow, not a workaround).

**Chose on-demand tools over indexing into ChromaDB, deliberately, after quantifying the actual trade-off — twice.** The question came up twice, once up front and once specifically about scaling ("what happens with too many docs"), and the honest answer wasn't obvious: Microsoft Graph's search endpoint hits Microsoft's own pre-built content index, so per-call latency (roughly 200ms–2s) does **not** degrade with how much content exists in OneDrive, only individual large-file downloads scale with size. That's the opposite of how a local ChromaDB index behaves, and it's what made "stay on-demand" a real engineering decision rather than a default.

**Five real bugs, found only by completing the actual live setup, not by writing the integration code:**
- A subprocess environment gap: the MCP server subprocess doesn't inherit the parent container's full environment by default (a curated safe subset only), so a correctly-set container env var never actually reached the code that needed it — confirmed by shelling into the live container, not by re-reading the spawn code. Fixed by passing exactly one additional variable through, deliberately not the full environment (which would have leaked unrelated API keys into a subprocess that has no need for them).
- An unrelated dependency-pinning gap the rebuild happened to surface: an unbounded version constraint let a fresh install pull a just-released major version that had renamed an exception class the app imported directly, breaking startup outright. Pinned properly — a real fix that had nothing to do with OneDrive itself, found only because this work forced a rebuild.
- Microsoft's own documentation presents a newer unified Search API as the general-purpose replacement for the older per-drive search endpoint. Live testing found it flatly rejects personal Microsoft accounts with an explicit error, despite the docs' framing — reverted to the older endpoint, confirmed it works correctly for a personal account, and corrected the project's own documentation rather than leaving the wrong claim in place.
- Two more environment/tooling gaps (a build-context exclusion blocking a one-time auth script, a config variable set in the example file but never the real one) — both found by actually running the setup a new user would run, not by reviewing the change.

**Verified against the real account, not a stub:** a real folder listing, a real 121-result search correctly distinguishing actual immigration-filing documents from unrelated HR-training mentions of the same keyword, and real text extracted from a live PDF download.

**Extended it further with a scoped, opt-in OCR-indexing script** — a specific OneDrive folder tree (not the whole drive) gets OCR'd and embedded into the same ChromaDB `search_docs` path, run manually rather than on a schedule. A real scaling bug was caught here too, by a sharp question about restart cost rather than by code review: the first-pass fix correctly stopped OneDrive-sourced chunks from being pruned, but still fetched the *entire* ChromaDB collection's metadata on every restart before filtering client-side — meaning restart cost would still have scaled with however much OneDrive content ever got synced in. Corrected by tagging every chunk with its origin at index time and filtering server-side, so the local knowledge base's own restart cost stays bounded regardless of OneDrive volume.

**Why this matters:** "authorize an app and call an API" is the easy 80%; the real engineering was in the parts that only show up against a live account and a live container — subprocess environment boundaries, a vendor's own documentation being wrong for one account type, and a scaling cost that only a restart-time question surfaces, not a code read.

---

### 25. Designing Safety for an Agent That Can Send Real Email — Outlook Integration

Extended the same Microsoft Graph integration to Outlook — read, compose, and send — which is a different risk category entirely from read-only OneDrive access: `send_email` is a genuinely irreversible action a tool-using agent can take.

**Two-step design, not a convenience split.** `compose_email` makes no network call at all — it's a pure local draft write that can never send anything by itself. `send_email` is the only call that can actually deliver mail. The safety design has two independent layers: a prompt-level rule (never call `send_email` in the same turn as `compose_email`; the original request to send is never itself treated as confirmation; only send after the user's *next* message explicitly confirms), and a mechanical backstop underneath it — an atomic database check that rejects any draft younger than 20 seconds, checked *before* the real send call, not after. The claim happens pre-send deliberately: if the send then fails, the draft stays claimed and can't be retried under the same id, trading a small inconvenience for never risking a duplicate send on retry.

**Live testing found a real, non-adversarial safety failure in the rejection message's own wording — and it's the most interesting finding in this whole integration.** The first version of the age-gate's rejection text ended with "...wait for the user's next message confirming they want it sent, then retry." Tested against the real deployed container with a real "yes, send it" typed immediately after composing — the model read "then retry" as license to immediately compose a **second** draft and send that one instead, right after being told to wait. Nothing adversarial fed that behavior; it came from completely ordinary phrasing. The mechanical age-gate still caught both attempts — nothing was ever actually sent — but it demonstrated a genuinely prompt-injection-shaped model behavior emerging with zero adversarial input, purely from an instruction that could be read two ways.

**Fixed by treating it as a wording bug worth re-testing, not just re-writing.** Reworded the rejection message and the system prompt's safety rule to explicitly forbid both retrying *and* recomposing-to-retry, then re-ran the identical scenario rather than assuming the reword worked. The result dropped from three tool calls to two on retry — a real, if partial, improvement, not a complete fix (the model still sometimes composes one redundant extra draft rather than purely waiting). Documented that residual behavior honestly as a conversational-compliance quirk rather than a safety gap, since the atomic per-draft age-gate blocks an actual send regardless of how many stray drafts a confused turn produces.

**Why this matters:** the finding here generalizes past this one feature — a safety instruction given to a language model is still just text the model can creatively reinterpret, including in ways nobody intended, with no adversarial input required. Wording a rule more carefully helps, but for anything genuinely irreversible, the real guarantee has to live outside the model, in a mechanism it can't argue its way around.
